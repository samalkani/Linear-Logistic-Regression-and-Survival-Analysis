# Linear, Logistic regression and Survival Analysis of the Addicts and Bladder data sets
...
# K. Create a logistic regression model with optimal cut-off

# I. Predicting the results
prob_pred = predict(logistic_model, type = 'response', newdata = addicts[c(2, 4:6)])
y_pred = ifelse(prob_pred > 0.4143900, 1, 0)

# II. Making the Confusion Matrix
cm = table(addicts[,3], y_pred > 0.5)
cm
...
# 14. B. Creation of a new variable based on multiplying the variable dose with the log of the survival
...
# 14. C. For the new variable DOSE=ln(DOSE)*T, which varies over time we print observation 106 
...
# 15. E. Creating a plot for individual with covariate pattern;  PRISON=1, DOSE=50 AND CLINIC=1
pct2=0:100/100
...
# 18. G. Gap Time approach, the starting time for patients at risk is set to zero 
